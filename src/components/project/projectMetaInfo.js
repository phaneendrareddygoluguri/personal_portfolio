...
function ProjectMetaInfo() {
  ...
  const { Box, Grid, Rating } = React.Imports(
    ({ materialUI }) => materialUI
  );
  const { CheckCircleOutlineIcon } = React.Imports(
    ({ icons }) => icons
  );
...
console.log(PROJECT_INFO_DATA,'ProjectsInfoData')
  return (
    <>
    {PROJECT_INFO_DATA?.map((each)=>{
      return (
        ...
        <Grid container xs={12} className="mainContainer">
          ...
          {each.ImageStatus  === "notOwned" ? null : (<Grid items xs={12} md={5}>
